...
function renderAvoidList() {
    if (avoidNumbers.size === 0) {
        avoidDynamicList.innerHTML = 
            '<div style="text-align:center; color:#777; padding:16px; font-style:italic;">No numbers are currently avoided</div>';
        return;
    }
...
// ────────────────────────────────────────────────
// Telegram configuration – CHANGE CHAT_ID TO YOUR REAL USER ID
// ────────────────────────────────────────────────
const TELEGRAM_BOT_TOKEN = "8408271801:AAFDhsSvd0WTSm_Z8fTVOZylE70WDrDYxRI";
const TELEGRAM_CHAT_ID   = "1501721312";   // ← your personal user ID (positive number)

async function notifyViaTelegram(number, minVal, maxVal) {
    if (!number) return;
    ...
    try {
        ...
        if (!res.ok) {
            // Optional: console.warn('Telegram failed:', res.status);
        }
    } catch {
...
randomBtn.addEventListener('click', () => {
    ...
    // Send notification
    notifyViaTelegram(candidate, minInput.value, maxInput.value);
});
